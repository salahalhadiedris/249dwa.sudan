...
def your_view(request):
    products = Product.objects.all()  # أو أي كويري سيت خاص بك
    
    # استخراج الفئات الفريدة من المنتجات المعروضة
    ...
    context = {
        'products': products,
        'unique_categories': categories_list,
    }
...
def product_list(request, category_slug=None):
    ...
    # دمج البيانات من قاعدة البيانات مع البيانات الثابتة
    enhanced_categories = []
    for db_category in categories:
        # البحث في البيانات الثابتة عن الفئة المناسبة
        static_data = next(
            (item for item in DRUG_CATEGORIES_DATA if item['name'] == db_category.name),
            None
        )
        ...
        if static_data:
            enhanced_categories.append({
                ...
                'icon': static_data['icon'],
                'views': static_data['views'],
                'color': static_data['color'],
                'db_object': db_category  # حفظ المرجع للكائن الأصلي
            })
        else:
            # استخدام قيم افتراضية إذا لم توجد بيانات ثابتة
            enhanced_categories.append({
                'id': db_category.id,
                'name': db_category.name,
                'slug': db_category.slug,
    ...
    context = {
        'categories': enhanced_categories,  # استخدام البيانات المدمجة
        'products': page_obj,
        'current_category': current_category,
    }
